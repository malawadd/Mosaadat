...
contract Pledges {
    ...
    function addPledge(
        ...
        require(
            handles[_handle].asks[_ask].pledges[totalPledges + 1].addr ==
                address(0)
        );

        uint256 totalAmount = handles[_handle].asks[_ask].totalAmount;
        uint32 pledgeIndex;
        ...
        if (handles[_handle].asks[_ask].pledgesByAddress[msg.sender] == 0) {
            ...
            pledgeIndex = handles[_handle].asks[_ask].pledgesByAddress[
                msg.sender
            ];
            require(
                handles[_handle].asks[_ask].pledges[pledgeIndex].amount <
                    _pledgeAmount
            );
        }
        ...
        handles[_handle].asks[_ask].pledgesByAddress[msg.sender] =
            totalPledges +
            1;
    ...
    function getAsk(bytes32 _handle, uint32 _ask)
        public
        view
        returns (
            bytes32 cid,
    ...
    function getPledge(
        ...
        uint32 _ask,
        uint32 _pledge
    ) public view returns (address addr, uint256 amount) {
        return (
